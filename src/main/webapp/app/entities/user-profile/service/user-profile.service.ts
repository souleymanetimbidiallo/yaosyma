...
type RestOf<T extends IUserProfile | NewUserProfile> = Omit<T, 'birthDate'> & {
...
export class UserProfileService {
  ...
  protected convertDateFromClient<T extends IUserProfile | NewUserProfile | PartialUpdateUserProfile>(userProfile: T): RestOf<T> {
    return {
      ...userProfile,
      birthDate: userProfile.birthDate?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restUserProfile: RestUserProfile): IUserProfile {
    return {
      ...restUserProfile,
      birthDate: restUserProfile.birthDate ? dayjs(restUserProfile.birthDate) : undefined,
    };
  }
