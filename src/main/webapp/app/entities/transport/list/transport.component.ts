...
import { EntityArrayResponseType, TransportService } from '../service/transport.service';
...
export class TransportComponent implements OnInit {
  ...
  trackId = (_index: number, item: ITransport): number => this.transportService.getTransportIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.transports = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: ITransport[] | null): ITransport[] {
    return data ?? [];
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    return this.transportService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
