...
public interface VehicleRepository extends JpaRepository<Vehicle, Long> {
    ...
    @Query(
        value = "select vehicle from Vehicle vehicle left join fetch vehicle.driver",
        countQuery = "select count(vehicle) from Vehicle vehicle"
    )
    Page<Vehicle> findAllWithToOneRelationships(Pageable pageable);

    @Query("select vehicle from Vehicle vehicle left join fetch vehicle.driver")
    List<Vehicle> findAllWithToOneRelationships();

    @Query("select vehicle from Vehicle vehicle left join fetch vehicle.driver where vehicle.id =:id")
